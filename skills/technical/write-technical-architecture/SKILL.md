...
1. Apply `$reason-technical`, `$reason-architecture`, and `$write-doc`.
...
9. When `$draw-diagrams` was explicitly invoked by the user, provide the accepted architecture meaning and let it own visual form. Otherwise retain sufficient prose or a table.
...
- Let `$write-architecture-knowledge` own reusable Architecture Knowledge.
