---
name: design-delivery-architecture
...
9. When creating or changing a visual, invoke `$draw-technical-architecture-diagrams` after defining its architecture meaning. Do not invoke it when no visual changes.
...
- Invoke `$write-decision-record` when one accepted choice needs an independent lifecycle or review trigger.
