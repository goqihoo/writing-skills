...
- Let the relevant artifact skill own document purpose and internal reasoning: `$write-knowledge` for reusable knowledge, `$write-prd` for product requirements, `$study-architecture` for reusable architecture knowledge, and `$design-architecture` for project-specific architecture.
