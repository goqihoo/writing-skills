...
3. Require an accepted product boundary. Return `$reason-product $write-doc` when the boundary is missing or materially disputed.
...
10. When relationships need a visual and `$draw-diagrams` was explicitly included, define the capability meaning before drawing; otherwise retain the smallest useful table or prose model.
...
- Own the whole-product capability model, not every capability detail; route an individual detail to `$write-product-doc $reason-product $write-doc`.
