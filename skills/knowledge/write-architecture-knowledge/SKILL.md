...
1. Apply `$reason-architecture` and `$write-doc`.
...
8. When `$draw-diagrams` was explicitly invoked by the user, provide the architecture meaning and let it own visual form. Otherwise retain sufficient prose or a table.
...
- Let `$write-technical-architecture` own one company's Technical Architecture.
