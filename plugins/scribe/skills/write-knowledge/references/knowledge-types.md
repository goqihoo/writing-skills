...
Infer the primary type from the artifact signals ordered in `SKILL.md`, then use the action readers need to perform after reading as a consistency check. A document may contain supporting material from other types, but its primary use controls the reasoning.
