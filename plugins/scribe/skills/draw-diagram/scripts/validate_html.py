#!/usr/bin/env python3
"""Validate a generated Scribe diagram HTML file, with no third-party deps.

Ships inside the skill so an installed agent can verify its own output:

    python3 <skill-dir>/scripts/validate_html.py my-diagram.html
...
def canonical_controller() -> str:
    if not MOTION_TEMPLATE.is_file():
        raise RuntimeError(
            ...
            "run validate_html.py from its shipped location inside the skill"
        )
