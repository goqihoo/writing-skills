...
**Both axes must carry the same scale and the same units.** That is the entire claim of the type: if the scales differ, the angle of every line is fiction. `../../../skills/draw-diagram/scripts/verify-slopegraph.py` gates it.
...
`../../../skills/draw-diagram/scripts/verify-slopegraph.py` requires all of them, cross-checks each visible string against its binding, and reports any label drawn nearer another series' endpoint than its own — a label on the wrong row renames the line.
