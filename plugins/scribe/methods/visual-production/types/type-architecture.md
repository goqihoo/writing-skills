...
**Rounded right-angle (orthogonal) connectors are MANDATORY** for all non-horizontal/vertical connections — diagonal `<line>` between off-axis nodes is a hard fail (see ../svg-guide.md `Connector grammar` Mandatory connector rules). Two-bend elbow path with `r=8`:
...
<rect x="{x}" y="{y}" width="{w}" height="{h}" rx="8"
      fill="rgba(34,38,58,0.02)" stroke="rgba(34,38,58,0.10)" stroke-width="0.8"/>
<rect x="{label_x}" y="{y+4}" width="{label_w}" height="12" rx="2" fill="{paper}"/>
<text x="{label_cx}" y="{y+13}" fill="rgba(34,38,58,0.40)" font-size="7"
...
- Zone fill: `rgba(34,38,58,0.02)` (2% ink wash). Any stronger competes with node fills.
- Max 3 zones per diagram. More and it reads like a swimlane (use that type instead).
- Dark mode: swap `rgba(34,38,58,…)` → `rgba(255,255,255,…)` same opacities; label mask fill = `paper` (dark).
...
- `../../../skills/draw-diagram/assets/examples/example-architecture.html`
