...
zones:                       # 2..4 zones, ordered along the orientation axis
  - name: "COLLECTION"
    components:              # 1..5 components per zone
      - id: survey-solutions
        ...
        sub:  "CAPI · PostgreSQL"
        icon: postgres               # any id from ../primitive-icons.md
  ...
  - name: "PROCESSING"
    components:
      ...
      - { id: sql-server,    name: "SQL Server",       sub: "on-premises · core RDBMS",                icon: sqlserver, color: "#007A59" }  # custom olive
  - name: "DISSEMINATION"
    components:
...
- `components[i][k].icon` — any id from `../primitive-icons.md`. If missing → no icon, the name shifts left. (Catalog has 41 icons; `mail` is currently missing — use `icon: file` as fallback for email hand-offs.)
...
**Icon placement** (24×24, monochrome via `currentColor` — see `../primitive-icons.md`):
...
`C_light` = the same hex lightened ~15 % for dark-mode contrast (e.g., `#007A59` → `#007A59`, `#D43E26` → `#D43E26`).
...
- `#D43E26` rust-red — security / governance / pain-point that isn't focal
- `#4F5BD5` slate-blue — observability / quality / monitoring gate
- `#007A59` olive-green — survivor system (the one tool the new platform keeps)
- `#B95D18` warm yellow — sandbox / dev / scratch
...
- **Using `kind: focal` to flag every painful thing.** Focal exists for ≤ 2 narrative pain-points; for "this is bad but not headline-bad", use `color: "#D43E26"` rust-red instead.

---

## Plotly specimen

- `../../../skills/draw-diagram/assets/examples/example-it-state.html`
...
zones:
  ...
  - name: "PROCESSING"
    components:
      - { id: shared-drive,  name: "Shared Drive",     sub: "No version control · Windows file share", icon: file,      kind: focal }
      - { id: analyst-mach,  name: "Analyst Machines", sub: "SPSS · SAS · Stata · Excel",              icon: desktop }
      - { id: sql-server,    name: "SQL Server",       sub: "on-premises · core RDBMS",                icon: sqlserver, color: "#007A59" }
  - name: "DISSEMINATION"
    components:
      - { id: legacy-portal,   name: "LegacyPortal",      sub: "manual bottleneck",     icon: cloud,    kind: focal }
      - { id: natstat-website, name: "NatStat Website",   sub: "public · static pages", icon: internet }
...
- SQL Server (custom olive) at zone 2, row 2: container fill `rgba(122,140,71,0.06)`, stroke `rgba(122,140,71,0.45)`, name text `#007A59` ✓
- Connectors 4, 5 (within zone 2) and 7, 8 (within zone 3) are simple vertical `<line>` elements. Cross-zone connectors take rule-compliant routes (see ../svg-guide.md `Connector grammar` rules 4 & 5):
