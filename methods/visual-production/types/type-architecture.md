...
**Rounded right-angle (orthogonal) connectors are MANDATORY** for all non-horizontal/vertical connections — diagonal `<line>` between off-axis nodes is a hard fail (see ../svg-guide.md Connector grammar). Two-bend elbow path with `r=8`:
