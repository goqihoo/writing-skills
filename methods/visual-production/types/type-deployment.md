...
2. **Infrastructure node** — a host, VM, pod, or managed service inside a zone. The node-box pattern in `../svg-guide.md` at `rx=6`, with a rectangular type tag (`rx=2`, **not** a pill) carrying `POD` / `VM` / `MANAGED` / `CDN` in the top-left corner.
...
- `../../../skills/draw-diagram/assets/examples/example-deployment.html`
