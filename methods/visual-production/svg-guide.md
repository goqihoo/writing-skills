...
Use one white canvas. Grouping containers use `paper-2`; ordinary objects use `object`; supporting elements may use transparent fill. A node consists of an opaque mask, its styled shape, an optional small rectangular type tag, a short name, and only necessary technical metadata.
...
Every connector label uses an opaque `paper` mask and sits 6–10 px away from its connector. Keep it to 14 characters when practical. Place labels beside vertical segments and never use vertical writing mode.
...
2. Keep 6–10 px of visible space between a connector and its label mask. The mask must not touch or cover the stroke.
...
6. Put connector labels on open canvas. A label mask must not overlap a node drawn after it.
...
The default budget is nine nodes, twelve connectors, and zero focal elements. A selected type reference may set a tighter or specialized budget. Split into overview and detail when the supported content exceeds it; do not shrink text or stack connectors to force a fit.
...
- the Plotly theme is the only color system and meaning survives without color;
- no default focal, active, selected, success, warning, or failure state has been invented;
