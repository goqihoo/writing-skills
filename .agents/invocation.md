...
`ask-scribe` is user-invoked. It is the human-facing index for the Scribe plugin. `write-doc` and `reason-architecture` are model-invoked foundations and stay out of normal router recommendations unless the user needs their shared discipline directly.
...
Express dependencies as prose invocation, such as “Apply the `/write-doc` skill.” Do not link across skill directories. A skill owns its own references and templates.
