...
class StructureDocsContractTest(unittest.TestCase):
    def test_ask_scribe_explains_without_executing_skills(self) -> None:
        ...
        metadata = (
            REPO_ROOT / "skills/ask-scribe/agents/openai.yaml"
        ).read_text(encoding="utf-8")
    ...
    def test_structure_docs_has_the_expected_name_and_plugin_entry(self) -> None:
        metadata = (
            REPO_ROOT / "skills/structure-docs/agents/openai.yaml"
        ).read_text(encoding="utf-8")
        ...
        self.assertTrue((REPO_ROOT / "skills/structure-docs/SKILL.md").is_file())

    def test_structure_docs_preserves_boundary_decisions(self) -> None:
        skill = (
            REPO_ROOT / "skills/structure-docs/SKILL.md"
        ).read_text(encoding="utf-8")
        patterns = (
            REPO_ROOT
            / "methods/documentation-structure.md"
        ).read_text(encoding="utf-8")
